...
%% plot
% patch('Faces',geom_rot.faces,'Vertices',geom_rot.vertices)
% axis equal
% view([1,1,1])
...
% [X_slice_mesh,Y_slice_mesh]=meshgrid(x_slices,fliplr(y_slices));
...
for count_current_slice=1:length(slice_sets)
        ...
        if ~slice_empty
                [loop_segments,closed_loops,num_sets]=getClosedLoops(plane_segment_ids);
                exist vertex1;
                if ans
